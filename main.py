...
app = FastAPI(title="Prophet Forecast API (multi-provider)", version="1.1.1")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def to_coinbase_product(symbol: str) -> str:
    ...
    for q in quotes:
        if s.endswith(q):
            base = s[:-len(q)]
            quote = "USD" if q == "USDT" else q
            return f"{base}-{quote}"
    # fallback
    return symbol.replace("/", "-")

# Coinbase allowed granularities (per docs): 60, 300, 900, 3600, 21600, 86400
def coinbase_granularity(interval: str) -> int:
    if interval == "1h":
        return 3600
    if interval == "4h":
        # 4h (14400) n'est pas listé officiellement; certaines régions l'acceptent,
        # sinon on peut approximativement utiliser 3600 et agréger côté client.
        return 14400
...
def fetch_kraken_klines(symbol: str, interval: str, limit: int = 300):
    pair = symbol.replace("BTCUSDT","XBTUSDT").replace("BTCUSD","XBTUSD").replace("BTC/EUR","XBTEUR")
...
def fetch_okx_klines(symbol: str, interval: str, limit: int = 300):
    instId = symbol.replace("BTCUSDT","BTC-USDT").replace("/", "-")
